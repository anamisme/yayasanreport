...
export async function onRequest(context) {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      await appendSheet(token, sheetId, `${SHEET_NAME}!A:D`, [
        nextId, body.nama, body.tarif || 0, now,
      ])
    ...
    if (method === 'PUT') {
      ...
      const body = await request.json()
      const data = await getSheet(token, sheetId, `${SHEET_NAME}!A:D`)
      const rows = data.values || []
      const rowIdx = rows.findIndex((r) => parseInt(r[0]) === parseInt(id))
      if (rowIdx < 0) {
        return new Response(JSON.stringify({ error: 'siswa PAUD tidak ditemukan' }), {
          status: 404,
          headers,
        })
      }
      await updateSheet(token, sheetId, `${SHEET_NAME}!A${rowIdx + 1}:D${rowIdx + 1}`, [
        parseInt(id), body.nama, body.tarif || 0, rows[rowIdx][3] || new Date().toISOString().split('T')[0],
      ])
    ...
    const data = await getSheet(token, sheetId, `${SHEET_NAME}!A:D`)
    let paud = parseRows(data.values || [])

    const search = url.searchParams.get('search')?.toLowerCase()
    if (search) {
      paud = paud.filter((s) => s.nama.toLowerCase().includes(search))
    }

    return new Response(JSON.stringify(paud), { headers })
  } catch (err) {
